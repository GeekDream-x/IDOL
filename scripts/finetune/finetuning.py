...
from ft_utils import convert_examples_to_features, processors
...
def evaluate(args, model, tokenizer, prefix="", test=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if not test:
            error_indices= []
            if len(out_label_ids) != len(preds):
                raise ValueError(f"len(prediction) != len(labels)")
                
            for i in range(len(preds)):
                if preds[i] != out_label_ids[i]:
                    error_indices.append(i)
            error_indices_filepath = os.path.join(eval_output_dir, "val_error_indices.txt")
            with open(error_indices_filepath, 'w') as error_writer:
                error_writer.write("\n".join([str(x) for x in error_indices]))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            cached_mode,
            '-'.join(list(filter(None, args.model_name_or_path.split("/")))[-2:]),
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        choices=["bert", "roberta", "albert", "debertav2"],
        required=True,
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        choices=["race", "swag", "arc", "reclor", "logiqa"]
        required=True,
