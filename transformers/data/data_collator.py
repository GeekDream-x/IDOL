...
def _torch_collate_batch(examples, tokenizer, pad_to_multiple_of: Optional[int] = None):
    ...
    if isinstance(examples[0], (list, tuple, np.ndarray)):
        ...
        new_examples = []
        for e in examples:
            new_examples.append(torch.tensor(e, dtype=torch.long))
...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if isinstance(examples[0], (dict, BatchEncoding)):
            ...
            new_batch = {"input_ids":[],"token_type_ids":[], "attention_mask":[], "logic_polarity_labels":[]}
            for sample in batch['input_ids']:
                new_batch['input_ids'].append(sample[0])
                new_batch['token_type_ids'].append(sample[1])
                new_batch['attention_mask'].append(sample[2])
                new_batch['logic_polarity_labels'].append(sample[3])
...
@dataclass
class DataCollatorForLanguageModelingBilingual(DataCollatorMixin):
    """
    Data collator used for language modeling. Inputs are dynamically padded to the maximum length of a batch if they
    are not all of the same length.
    ...
    tf_experimental_compile: bool = False
    return_tensors: str = "pt"
    reset_position_ids_for_bilingual_pair: bool = True
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if self.reset_position_ids_for_bilingual_pair is True:
            padding_idx = self.tokenizer.pad_token_id
            mask1 = batch['input_ids'].ne(padding_idx).int()
            incremental_indices = (torch.cumsum(mask1, dim=1).type_as(mask1))  #* mask1
            total_length = batch['input_ids'].size(1)
            for idx, example in enumerate(examples):
                length1 =example['input_ids'].index(2) + 1 # length of the first sentence
                position2 = torch.arange(total_length,dtype=torch.int) - length1 + 1
                mask2 = (position2 < 1).int()
                incremental_indices[idx] = (incremental_indices[idx] * mask2 + position2*(1-mask2))

            batch['position_ids'] = (incremental_indices * mask1 + padding_idx).long()
        return batch

    /raw7/rc/zhxu13/reclor/For_Public
    def torch_mask_tokens(self, inputs: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
