...
@add_start_docstrings("""DeBERTa Model with a `language modeling` head and a logic prediction head on top. """, DEBERTA_START_DOCSTRING)
# Copied from transformers.models.deberta.modeling_deberta.DebertaForMaskedLM with Deberta->DebertaV2
class DebertaV2ForLogicMaskedLM(DebertaV2PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids", r"predictions.decoder.bias"]
    ...
    def __init__(self, config, lcp_weight):
        ...
        self.deberta = DebertaV2Model(config)
        self.cls = DebertaV2OnlyMLMHead(config)
        self.lg_head = DebertaV2LogicPredictionHead(config, 6)
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        prediction_scores = self.cls(sequence_output)
        lg_prediction_scores = self.lg_head(sequence_output)
        ...
        return MaskedLMOutput(
            loss=total_loss,
            logits=(prediction_scores, lg_prediction_scores),
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        )
...
class DebertaV2LogicPredictionHead(nn.Module):
    def __init__(self, config, lpp_version):
        super().__init__()
        self.transform = DebertaV2PredictionHeadTransform(config)

        self.decoder = nn.Linear(config.hidden_size, lpp_version, bias=False)
        self.bias = nn.Parameter(torch.zeros(lpp_version))
